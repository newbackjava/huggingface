# filename: ko2en_no_drop_print_py39.py
...
INPUT_PATH  = "input.txt"        # 원문 파일 경로
OUTPUT_PATH = "output_en.txt"    # 번역 결과 저장 경로
...
# 출력 길이 상한(선택): 영어가 길어지면 400~600으로 확대 가능
MAX_NEW_TOKENS = 300
...
def split_line_to_sentences(line: str) -> List[str]:
    """
    한 줄을 문장들로 분해. 번호 목록은 가급적 한 문장으로 유지.
    그 외에는 종결부호 기준 간단 분리.
    """
    if is_numbered_line(line):
        return [line]
    parts = re.split(r"(?<=[\.!?])\s+", line)  # 마침표/물음표/느낌표 뒤 공백
...
def translate_segments(ko2en_pipe, segments: List[str]) -> List[str]:
    """
    세그먼트 리스트를 배치로 번역.
...
def main():
    ...
    # 번역
    translated_segments = translate_segments(ko2en, segments)
